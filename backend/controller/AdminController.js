...
const manageleavefunction = async (req, res) => {
  try {
    ...
    const currentLeaveBalance = await UserModel.find({
    ...
    if (newValue == "approve") {
      let UpdatedLeaveBalance =
        currentLeaveBalance[0].leaveBalance - total_leave_days;
    ...
    if (newValue == "reject") {
      let UpdatedLeaveBalance =
        Number(currentLeaveBalance[0].leaveBalance) + Number(total_leave_days);
