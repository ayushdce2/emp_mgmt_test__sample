...
const applyleavefunction = async (req, res) => {
  ...
  if(fromDate < currentDate || toDate < currentDate || toDate < fromDate || (difference_leaveDays+1)<0){
            // console.log("Dates are mismatch, check again")
            res.status(409).json({ success: false, message: "Dates are mismatch, check again" });
...
//    console.log("insufefircient");
   res.status(409).json({ success: false, message: "Insufficient Leave Balance" });
            ...
            // console.log("good to go");
              const existingLeave = await LeaveModel.findOne({ email,date_from: { $lte: toDate },date_to: { $gte: fromDate } });
            //   console.log(existingLeave,"existingLeave")

              if(existingLeave){
            // console.log("Already exists");
             res.status(409).json({ success: false, message: "This time period already exists" });
        }

        if (!existingLeave ) {
        //  console.log(leaveType,"<=============leaveType")
            const submitLeave = new LeaveModel({ total_leave_days:difference_leaveDays+1,email, name,date_from:fromDate,date_to:toDate,leave_type:leaveType,leave_reason:reason });
...
const {email,_id}= req.user;
    try{
        ...
        const AllLeavesDetails = await LeaveModel.find({email}).sort({ createdAt: -1 });
        // console.log(AllUserDetails)
...
module.exports = {applyattendancefunction,attendancehistoryfunction, applyleavefunction,getLeaveSummary};